			...
			Inhabitant.State.TRABAJANDO:
				if _works_at_camp(person):
					...
					if person.job == Profession.Job.MANUFACTURA:
						...
						_harvest(person, _creel_round(person, hours))
...
func _do_ascent(person: Inhabitant) -> void:
	...
	# pocas veces.
	tell_tale(Tale.discovery("Cumbre coronada",
		...
		person.position, day, person.current_task()))
...
func _tend_camp(person: Inhabitant, hours: float) -> void:
	...
	var smoking: bool = (hearth_lit
		and camp_built.get(CampProjects.Kind.SECADERO, false)
...
func _smoke_the_larder() -> void:
	...
	_dry_meat(supervision, AHUMADO_BONUS)
...
func _best_known_spot(person: Inhabitant) -> Vector3:
	...
	if person.current_speciality == Profession.Speciality.CAZA_MENOR \
			or person.current_speciality == Profession.Speciality.CAZA_MAYOR:
		var abierta := kill_to_fetch_near(person.position, 1200.0)
		if abierta != null:
	...
	match hunt.phase:
		...
		_:
			...
			if hunt.crew.is_empty():
				return 0.0
			return clampf(hunt.spent / maxf(
				_tracking_hours(hunt.crew[0]), 0.01), 0.0, 1.0)
...
func _check_hunting_risk(person: Inhabitant, speciality: Profession.Speciality,
	...
	var risk := Hunting.risk_at(speciality, person.work_centre,
		GameState.season as Subsistence.Season, hunters_in(speciality), toolkit)
	if risk <= 0.0:
		return
	risk *= fraction
	if _rng.randf() > risk:
	...
	var species := Fauna.huntable_at(person.work_centre,
		GameState.season as Subsistence.Season, porte as Fauna.Porte, toolkit)
...
func _check_quarry_risk(person: Inhabitant, hunt: Hunt) -> void:
	...
	risk *= LANCE_RIESGO
	if risk <= 0.0 or _rng.randf() > risk:
		return
...
## jornada entera. Ver `_check_hunting_risk`, que ya escala por eso mismo.
...
func _huntable_species(speciality: Profession.Speciality) -> Array:
	...
	for species: String in Fauna.SPECIES:
		if Fauna.porte_of(species) > porte:
			continue
		if not Fauna.huntable_with(species, toolkit):
			continue
		out.append(species)
	return out
...
func _open_hunt(person: Inhabitant) -> Hunt:
	...
	hunt.weapon = Fauna.weapon_at_hand(hunt.species, toolkit)
	hunts.append(hunt)
	return hunt
...
## `_quarry_bonus` para elegir coto, aplicada al animal concreto.
...
## `HORAS_UTILES / pieces` horas de jornada perfecta; lo que la persona tenga
...
func _tracking_hours(person: Inhabitant) -> float:
	var speciality := person.current_speciality as Profession.Speciality
	var pieces := Hunting.pieces_per_day(speciality, techs)
	if pieces <= 0.0:
		return HORAS_UTILES

	var chain := person.effectiveness() * _knowledge_factor(person)
	chain *= ResourceField.seasonal_factor(person.activity, GameState.season)
	chain *= weather.work_factor()
	chain *= Hunting.crew_factor(speciality, hunters_in(speciality))
	var tool_kind := _tool_for(person)
	if tool_kind >= 0:
		chain *= toolkit.efficiency(tool_kind as Tool.Kind,
			workers_in(person.activity))
	if chain <= 0.01:
		return HORAS_UTILES

	return HORAS_UTILES / (pieces * chain)
...
func _hunt_step(person: Inhabitant, hours: float) -> void:
	...
	if hunt == null:
		var pendiente := kill_to_fetch_near(person.position, ALCANCE_DEL_TAJO * 3.0)
		if pendiente != null:
...
func _stalk(person: Inhabitant, hunt: Hunt, hours: float) -> void:
	...
	if distance > Hunt.PIERDE_M or hunt.spent > Hunt.ACECHO_HORAS:
		...
		return
	var reach := Hunt.reach_of(hunt.weapon, techs)
	...
	if techs != null and techs.has(TechTree.Tech.OJEO):
...
func _chase(person: Inhabitant, hunt: Hunt, hours: float) -> void:
	...
	if distance <= Hunt.reach_of(hunt.weapon, techs) * Hunt.TIRO_A_LA_CARRERA:
		hunt.phase = Hunt.Phase.LANCE
...
## sigues. El riesgo VIVIA en `_harvest`, y al dejar de pasar la caza por ahí
...
## que `_check_hunting_risk` vino a arreglar en su dia.
##
...
func _throw(person: Inhabitant, hunt: Hunt) -> void:
	_check_quarry_risk(person, hunt)
	...
	if hunt.weapon >= 0:
		toolkit.use(hunt.weapon as Tool.Kind,
			Tool.wear_per_day(hunt.weapon as Tool.Kind) * 0.5)
		_note_breakage(person, hunt.weapon as Tool.Kind)

	if _rng.randf() > clampf(chance, 0.02, 0.95):
	...
	if field:
		var cell := field.best_cell(person.activity, person.position,
			ALCANCE_DEL_TAJO)
		field.take_from_cell(person.activity, cell,
			Fauna.rations_of(hunt.species) * DEPLETION_PER_UNIT)
	...
	_note(Chronicle.Kind.TIERRA, "%s cobró %s en %s. Toca %s." % [
		person.given_name, Fauna.species_name(hunt.species).to_lower(),
		parajes.place_name(hunt.kill_site, home_position), how],
...
func _butcher_in_field(person: Inhabitant, hunt: Hunt, hours: float) -> void:
	...
	if person.position.distance_to(hunt.kill_site) > arrive_radius * 2.0:
		person.log_deed(person.current_task(),
	...
	var edge := toolkit.efficiency(Tool.Kind.LASCA, hunt.crew.size())
	var fraction := hours / HORAS_UTILES
	toolkit.use(Tool.Kind.LASCA, fraction * Tool.wear_per_day(Tool.Kind.LASCA))
	_note_breakage(person, Tool.Kind.LASCA)

	hunt.spent += fraction * edge
	person.log_deed(person.current_task(), hunt.doing_text(), false)
	if hunt.spent < Hunt.butcher_days(hunt.species):
		return
...
## La pieza abierta más cercana que todavía tiene carga, o null.
##
...
func kill_to_fetch_near(point: Vector3, radius: float) -> Hunt:
	...
	var best_distance := radius
	for hunt: Hunt in hunts:
		if hunt.phase != Hunt.Phase.ACARREO or hunt.spoils.is_empty():
			continue
...
##
## Menos que trampas de monte -[TRAMPAS_POR_TRAMPERO] son cinco- por dos
...
func can_set_nasas() -> bool:
	if techs == null or not techs.has(TechTree.Tech.NASA):
		return false
	return toolkit.count(Tool.Kind.NASA) > 0
...
func _room_for_nasa(point: Vector3) -> bool:
	if nasas.size() >= nasa_allowance():
		return false
	if _terrain != null and not _water_beside(point):
		return false
...
## hay que ir hoy, igual que [_fullest_trap] en el monte.
...
## quien llama se ponga a pescar de verdad con `_harvest`.
##
...
func _creel_round(person: Inhabitant, hours: float) -> float:
	...
	if full != null:
		...
		if pieces > 0:
			...
			_note(Chronicle.Kind.TIERRA, "%s levantó la nasa de %s: %d piezas."
				% [person.given_name,
					parajes.place_name(full.position, home_position), pieces], 0)
	...
	var piece := toolkit.detach(Tool.Kind.NASA)
	if piece == null:
		return hours
...
func _spend(hours: float, day_fraction: float) -> float:
	return maxf(hours - day_fraction * HORAS_UTILES, 0.0)
		...
		if nasa.is_spent():
			...
			_note(Chronicle.Kind.PENURIA,
				"La nasa de %s se ha podrido en %s. Dio %d piezas."
					% [nasa.maker,
						parajes.place_name(nasa.position, home_position),
						nasa.taken], 0)
			continue
		alive.append(nasa)
	nasas = alive
