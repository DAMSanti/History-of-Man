...
# --- LAS FICHAS DEL SITIO: que hay donde has pinchado ---------------------
#
# Diecisiete funciones que estaban en tres sitios del fichero. Solo se mueven
# lineas.
#
# Todas responden a la misma pregunta desde angulos distintos: que hay en este
# punto del mapa. El suelo desnudo, un paraje con nombre, una cumbre, un
# recurso del suelo, una boca de cueva, o la lista entera de sitios conocidos.
...
func _climber_row(body: VBoxContainer, person: Inhabitant,
	...
	label.text = "%s · %d%% de ascensión" % [
		barra._person_card_line(person), int(person.skill_in(task) * 100.0)]
	label.add_theme_font_size_override("font_size", 11)
