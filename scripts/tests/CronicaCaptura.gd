extends SceneTree
## La pestaña de Crónica abierta, para poder mirarla.
##
## Una ventana no se juzga con una cuenta: se juzga viéndola. Aquí se juega unas
## jornadas, se abre la ficha de un explorador, se pulsa la pestaña y se
## dispara.
##
...
func _init() -> void:
	...
	# Un explorador, que es por donde se pidió empezar.
	...
	ui._pestana_persona = ui.PestanaPersona.CRONICA
	...
	if shot != null:
		shot.save_png("user://cronica.png")
		print("captura en user://cronica.png · %s" % quien.given_name)
	quit()
