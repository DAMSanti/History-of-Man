...
# --- LA MARCHA: rutas, pasos y atascos -----------------------------------
#
# Estas veintitres funciones estaban repartidas por doce sitios del fichero.
# Juntarlas NO cambia nada de lo que hace el juego -este commit solo mueve
# lineas- pero es lo que hace posible sacarlas luego de una pieza: un corte
# contiguo es una operacion segura y doce cortes sueltos son doce sitios donde
# equivocarse. Ver ARQUITECTURA §3, regla 2.
#
# Lo que hay aqui es la capa de MOVIMIENTO: trazar la ruta, guardarla, andarla
# paso a paso, elegir carril para no ir todos en fila, saber a que velocidad se
# pisa cada terreno, y vigilar a quien no avanza.
